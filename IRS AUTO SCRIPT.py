...
URLS = [
    ...
    "https://connect.irs.gov/system/templates/chat/aur_nlp_va_en/index.html?entryPointId=1001&locale=en-US&postChatAttributes=false&templateName=aur_nlp_va_en&ver=v11&VAEnabled=true&vaChatEntryPointId=&vaChatServerURL=&VATenantAccId=TMPROD10067889&VAName=aurenprod&ShowPreChatOnEscalation=&serverURL=https://connect.irs.gov/system&vaLastAvatar=&providerId=186A7&wsname=https://www.irs.gov&EGAIN_AV_CHAT_STATE_DATA=null&parentLost=false&referer=https://www.irs.gov/individuals/understanding-your-cp2000-series-notice&useCustomButton=false&storage=true&docked=true",
    "https://connect.irs.gov/system/templates/chat/testpages/aur_nlp_va_en.html",
...
def _recipient_chips(emails: List[str], max_show: int = 4) -> str:
    ...
    if remaining > 0:
        chips += '; <span class="more">+' + str(remaining) + " others</span>"
    return chips
    ...
    try:
        if system == "Windows":
            ...
            if browser:
                ...
                subprocess.Popen([browser, "--new-window", draft_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                time.sleep(1.8)
                batch_size = 4
        ...
        if system == "Darwin":
            subprocess.Popen(["open", "-n", draft_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            time.sleep(1.2)
            for u in urls:
